...
public class Voflix extends Spider {
    ...
    private static final String siteUrl = "https://www.voflix.me";
    ...
    public String homeContent(boolean filter) {
        ...
        for (Element element : doc.select(".navbar > ul > li > a")) {
            if (element.attr("href").startsWith("/type")) {
                String id = element.attr("href").replaceAll("\\D+", "");
                String name = element.text();
                classes.add(new Class(id, name));
            }
        }
        for (Element element : doc.select("div.module:nth-child(3) > div:nth-child(2) > div:nth-child(1) > a")) {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        List<Vod> list = new ArrayList<>();
        String target = siteUrl + String.format("/show/%s--------%s---.html", tid, pg);
    ...
    public String detailContent(List<String> ids) {
        ...
        Elements sourceList = doc.select("div.module-list > div");
    ...
    public String searchContent(String key, boolean quick) {
        List<Vod> list = new ArrayList<>();
        String target = siteUrl.concat("/search/-------------.html?wd=").concat(key);
        Document doc = Jsoup.parse(OkHttp.string(target, getHeaders()));
