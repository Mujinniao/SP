...
public class Nailu extends Spider {


    private static final String siteUrl = "https:///www.newfii.com";
    ...
    public String homeContent(boolean filter) {
        ...
        for (Element element : doc.select(".nav > ul > li a")) {
        ...
        for (Element element : doc.select("div.module-item div.module-item-cover div.module-item-pic")) {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        ...
        for (Element element : doc.select("div.module-item-cover div.module-item-pic")) {
            String img = element.select("img").attr("data-src");
            String name = element.select("a").attr("title");
            String id = element.select("a").attr("href").replaceAll("\\D+","");
            list.add(new Vod(id, name, img));
    ...
    public String detailContent(List<String> ids) {
        ...
        String type = doc.select(".video-tag-icon").text();
        String actor = doc.select("div.video-info-items:nth-child(2) > div:nth-child(2) > a").text();
        String content = doc.select(".video-info-content > span:nth-child(1)").text();
        String director = doc.select("div.video-info-items:nth-child(1) > div:nth-child(2) > a").text();
        ...
        Elements sources = doc.select("div.module-tab:nth-child(2) > div:nth-child(3) > div.module-tab-content > div");
        Elements sourceList = doc.select("div.module-blocklist > div:nth-child(1)");
        for (int i = 0; i < sources.size(); i++) {
            Element source = sources.get(i);
            String sourceName = source.select("span").text();
    ...
    public String searchContent(String key, boolean quick) {
        List<Vod> list = new ArrayList<>();
        String target = siteUrl.concat("/vodsearch/-------------/?wd=").concat(key);
        Document doc = Jsoup.parse(OkHttp.string(target, getHeaders()));
        for (Element element : doc.select("div.module-search-item")) {
            ...
            String remark = element.select("div:nth-child(2) > div:nth-child(1) > a:nth-child(1)").text();
