...
public class SanWu extends Spider {


    private static final String siteUrl = "https://www.555yy7.com";
    ...
    public String homeContent(boolean filter) {
        ...
        for (Element element : doc.select(".navbar > ul > li > a")) {
            if (element.attr("href").startsWith("/vodtypehtml")) {
                String id = element.attr("href").replaceAll("\\D+", "");
                String name = element.attr("title");
                classes.add(new Class(id, name));
            }
        }
        for (Element element : doc.select("div.module-main > div > a")) {
            String img = element.select("div.module-item-pic > img").attr("data-original");
            String name = element.attr("title");
            String remark = element.select("div.module-item-note").text();
            String id = element.attr("href").replaceAll("\\D+","");
    ...
    public String detailContent(List<String> ids) {
        ...
        String remarks = doc.select("div.module-info-item:nth-child(10) > a:nth-child(2) > font:nth-child(1)").text();
        String img = doc.select(".ls-is-cached").attr("data-original");
        String type = doc.select("div.module-info-tag-link:nth-child(3)").text();
        String actor = doc.select("div.module-info-item:nth-child(6) > div:nth-child(2)").text();
        String content = doc.select(".module-info-introduction-content > p:nth-child(1)").text();
        String director = doc.select("div.module-info-item:nth-child(4) > div:nth-child(2)").text();
