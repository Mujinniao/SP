...
public class Mianfk extends Spider {


    private static final String siteUrl = "https://qzy998-1.ysgq.xyz";
    ...
    public String homeContent(boolean filter) {
        ...
        for (Element element : doc.select(".nav-channel > a")) {
            if (element.attr("href").startsWith("/index")) {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        List<Vod> list = new ArrayList<>();
        String target = siteUrl + String.format("/index.php/vod/show/id/%s/page/%s.html", tid, pg);
    ...
    public String detailContent(List<String> ids) {
        Document doc = Jsoup.parse(OkHttp.string(siteUrl.concat("/index.php/vod/detail/id/").concat(ids.get(0)), getHeaders()));
        ...
        String type = doc.select("p.item:nth-child(5) > a").text();
    ...
    public String searchContent(String key, boolean quick) {
        List<Vod> list = new ArrayList<>();
        String target = siteUrl.concat("/index.php/vod/search.html?wd=").concat(key);
        Document doc = Jsoup.parse(OkHttp.string(target, getHeaders()));
        for (Element element : doc.select(".search-list")) {
            String img = element.select(".bj").attr("data-original");
            String name = element.select(".aplus-exp").attr("title");
            String remark = element.select("a:nth-child(2) > div:nth-child(1) > div:nth-child(1)").text();
            String id = element.select(".aplus-exp").attr("href").replaceAll("\\D+","");
            list.add(new Vod(id, name, img, remark));
        }
        return Result.string(list);
    }
